...
export class MpesaService {
  ...
  // Safaricom Sandbox Shortcode and Passkey
  ...
  /**
   * Generates a Bearer token from Safaricom Daraja API
   */
  async getAccessToken(): Promise<string> {
    ...
    try {
      const response = await lastValueFrom(
        this.httpService.get(this.getAuthUrl(), {
          headers: {
  ...
  /**
   * Initiates the STK Push prompt to the user's phone
   */
  async stkPush(userId: string, dto: StkPushDto) {
    ...
    // This URL receives Safaricom's callback. Ensure it is publicly accessible.
    ...
    try {
      const response = await lastValueFrom(
        this.httpService.post(this.getStkPushUrl(), payload, {
          headers: {
  ...
  /**
   * Processes the Webhook callback from Safaricom
   */
  async processCallback(payload: any) {
    ...
    // In a real production app, you would look up the user by matching the CheckoutRequestID
    // saved in the DB earlier. For MVP, we will try to extract the user from the AccountReference
    // if possible, or just log the successful receipt.
    this.logger.log(`Payment of KES ${amount} successful! Receipt: ${receiptNumber}`);

    // Assuming we somehow mapped CheckoutRequestID or AccountReference back to the User ID.
    // For MVP demonstration, we just log it. A robust DB table is needed for active tracking.
